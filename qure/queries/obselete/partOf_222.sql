...
--SELECT count(*) FROM
--(
SELECT DISTINCT OV.gid, OV.block, TID.block
FROM :bt_name AS TID,
     (
      SELECT DISTINCT OV.gid, BT.block
      FROM :bt_name BT,
           (
      ...
      (
       ...
       WHERE TID.gid = :id_val AND
             ...
             (
              ...
              WHERE (TID.block & :lh) >= V.n AND
                    (OV.block = ((TID.block & ~((1::bigint << ((:len_b - V.n) - 1)) - 1)) | V.n) OR
                     OV.block = TID.block)
            )
     );
