CREATE OR REPLACE FUNCTION bmbt_po1_opt(bm text, bt_name text) RETURNS integer AS $$
DECLARE
  id_val integer;
  len_s integer;
  len_v integer;
     ...
     (
      ...
      WHERE T1.gid = ' || id_val || ' AND 
            ((T1.block <= T2.block AND
              (((1::bigint << (63 - (T1.block & 63)::int)) - 1) | T1.block) >= T2.block) OR
             ((T1.block & 63) >= V.n AND
              T2.block = ((T1.block & ~((1::bigint << ((64 - V.n) - 1)) - 1)) | V.n)) OR
             T2.block = 0)
     ) AS OV
    WHERE NOT EXISTS
      (
       ...
       WHERE TOV.gid = OV.gid AND
             ...
             (
              ...
              WHERE TID.gid = ' || id_val || ' AND
                    ...
                    TOV.block <= (((1::bigint << (63 - (TID.block & 63)::int)) - 1) | TID.block)
             )
      )';
  END LOOP;
  RETURN 1;
